...
contract PoolControllerFacet is Modifiers {
    ...
    function collectAllFeesXinfin(
        ...
        Constants.NON_FUNGIBLE_POSITION_MANAGER.safeTransferFrom(
            msg.sender,
            address(this),
            tokenId
        );
        ...
        (amount0, amount1) = Constants.NON_FUNGIBLE_POSITION_MANAGER.collect(
            params
        );
    ...
    function decreaseAllLiquidity(
        ...
        require(
            msg.sender == s.depositXswap[tokenId].owner ||
                msg.sender == s.admin,
            "Not the owner"
        );
    ...
    function increaseLiquidityCurrentRangeXinfin(
        ...
        TransferHelper.safeTransferFrom(
            s.depositXswap[tokenId].token0,
            msg.sender,
            address(this),
            amountAdd0
        );
        TransferHelper.safeTransferFrom(
            s.depositXswap[tokenId].token1,
            msg.sender,
            address(this),
            amountAdd1
        );
        ...
        TransferHelper.safeApprove(
            s.depositXswap[tokenId].token0,
            address(Constants.NON_FUNGIBLE_POSITION_MANAGER),
            amountAdd0
        );
        TransferHelper.safeApprove(
            s.depositXswap[tokenId].token1,
        ...
        (liquidity, amount0, amount1) = Constants
            .NON_FUNGIBLE_POSITION_MANAGER
    ...
    function retrieveNFTXinfin(uint256 tokenId) external {
        ...
        INonfungiblePositionManager(s.nonfungiblePositionManager)
