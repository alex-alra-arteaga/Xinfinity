...
contract PoolController is Modifiers, IERC721Receiver {
    ...
    function _createDeposit(address owner, uint256 tokenId) internal {
        ...
        s.depositsXinfinity[tokenId] = Types.Deposit({
            owner: owner,
            liquidity: liquidity,
            token0: token0,
            token1: token1
    ...
    {
        require(
            s.poolRegistry[token0][token1][poolFee] != address(0),
            "PoolController: Pool does not exist"
        );
        require(
            tickDesired > TickMath.MIN_TICK && tickDesired < TickMath.MAX_TICK,
            "PoolController: Invalid tick"
        );
        ...
        INonfungiblePositionManager.MintParams memory params = INonfungiblePositionManager
            .MintParams({
                ...
                token1: token1,
                fee: poolFee,
                tickLower: tickDesired -
                    ((Constants.LIQUIDITY_WIDTH * tickDesired) / 1000), // 10 % of the tickDesired
                tickUpper: tickDesired +
                    ((Constants.LIQUIDITY_WIDTH * tickDesired) / 1000), // 10 % of the tickDesired
    ...
    function collectAllFees(
        ...
        INonfungiblePositionManager(s.xinfinityManager).safeTransferFrom(
            msg.sender,
            address(this),
            tokenId
        );
        ...
        (amount0, amount1) = INonfungiblePositionManager(s.xinfinityManager)
            .collect(params);
    ...
    function retrieveNFT(uint256 tokenId) external {
        ...
        require(
            msg.sender == s.depositsXinfinity[tokenId].owner,
            "Not the owner"
        );
    ...
    function decreaseNecessaryLiquidity(
        uint256[] memory tokenIds,
        uint256 amount,
        bool isZeroToken
    ) external returns (uint256 totalAmount0, uint256 totalAmount1) {
        // Initial check to ensure the caller owns all the tokenIds
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                msg.sender == s.depositsXinfinity[tokenIds[i]].owner,
                "Not the owner of all tokenIds"
            );
        }
    ...
    function increaseLiquidityCurrentRange(
        ...
        TransferHelper.safeTransferFrom(
            s.depositsXinfinity[tokenId].token0,
            msg.sender,
            address(this),
            amountAdd0
        );
        TransferHelper.safeTransferFrom(
            s.depositsXinfinity[tokenId].token1,
            msg.sender,
            address(this),
            amountAdd1
        );
